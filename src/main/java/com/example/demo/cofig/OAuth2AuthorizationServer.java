 ...
 * security) throws Exception { security .tokenKeyAccess("permitAll()")
 * .checkTokenAccess("isAuthenticated()") .allowFormAuthenticationForClients();
 * }
 * 
 * @Override public void configure(ClientDetailsServiceConfigurer clients)
 * throws Exception { clients .inMemory() .withClient("clientapp")
 * .secret(passwordEncoder.encode("123456")) .authorizedGrantTypes("password",
 * "authorization_code", "refresh_token") .authorities("READ_ONLY_CLIENT")
 * .scopes("read_profile_info") .resourceIds("oauth2-resource")
 * .redirectUris("https://sso.login.run-np.homedepot.com/")
